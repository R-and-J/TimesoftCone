...
9b. **Leave master is this system** — `leave_balance` is owned internally too. Leave granting/deduction goes through `LeaveGrantPort`, default impl `InternalLeaveAdapter`. The leave **request/approval (기안/승인) workflow** is out of scope (external groupware owns it). What *is* implemented is the priority-deduction *rule* (`UseLeaveUseCase` + `planLeaveDeduction`, ADMIN-only endpoint) so the inv #3 deduction order (AUCTION→EVENT→REGULAR) lives authoritatively in code and is ready when an external trigger integrates. The year-end pool collection is a *separate bounded context* (`LeavePool`), not part of day-to-day leave management. (ADR-016, ADR-017)
...
12. **Value Objects for core types** — *As built*, `domain/shared/value-objects/` has `UserId`, `Cone` (the points VO — not "Point"), `AuctionId`, `Currency` as classes with invariants enforced at construction. `LeaveType`/`AuctionStatus`/`LedgerActionType` are string-union types + guards, not VO classes; ADR-015's `LeaveDays`/`Year` were not wrapped (still raw `number`/`int`). Don't assume a VO exists for a type without checking the folder. (ADR-015)
