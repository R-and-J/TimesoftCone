...
| **FR-2.1** | 실시간 입찰 진행 | 잔여 포인트 내 입찰, 상위 입찰 발생 시 기존 최고가 입찰자에게 WebSocket 알림 발송 | 포인트 부족 시 입찰 버튼 비활성화 / **[제약]** MySQL InnoDB 행 락(`SELECT … FOR UPDATE`)으로 동시성 제어 (CUT-1) |
...
마감 직전 다수 사용자의 동시 입찰 시 발생하는 경쟁 상태(Race Condition)를 제어하기 위해 **MySQL InnoDB 행 락**(`SELECT … FOR UPDATE`, 트랜잭션 수명 동안 보유)을 적용하여 단일 입찰가 정합성을 **100% 보장**한다.
