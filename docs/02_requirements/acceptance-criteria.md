...
- SQLite write 락(`lockAuction` no-op UPDATE) — 트랜잭션 수명 동안 보유 (CUT-1, [ADR-006](../04_decisions/ADR-006-redis-lock.md)은 Superseded)
...
| AC-2.1.E4 | 동시 입찰(같은 경매) | 두 입찰 동시 도착 | SQLite write 락으로 직렬화 — 뒤 입찰은 앞 입찰 커밋 후 갱신된 현재가로 재검증(`BID_TOO_LOW` 가능) |
...
### NFR-1 동시성 (SQLite write 락)

| ID | Given | When | Then |
|---|---|---|---|
| AC-N1.1 | 경매 X에 1000명이 동시 입찰 시도 | 동시 입찰 | 한 번에 1명만 통과 (SQLite write 락 직렬화) · 나머지는 락 해제 후 순차 진입하여 갱신된 현재가로 재검증 · 입찰가 꼬임 0건 |
