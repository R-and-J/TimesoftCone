...
**부분 부활 (2026-05-20)**: `users.regular_leave_days` / `auction_leave_days` / `event_leave_days` 컬럼 3개 추가 + `GET /api/users/:id/leave` 읽기 전용 엔드포인트. Dashboard와 MyActivity의 휴가 카드가 실제 사용자별 값으로 표시됨.
...
**✅ 배당 지급 부활 (2026-05-27)**: 예상치(projection)만 있던 배당에 **실제 지급 배치**를 추가 — `PayoutChannel` 포트(ADR-010 ISP 분리) + `SettleYearEndDividendUseCase`. stake 비례 floor 배당 + 나머지 1위 몰아주기로 `Σ배당 = escrow`(NFR-2) 정확히 성립, 멱등(재호출 409), `POST /api/admin/dividend/settle?dryRun=`. 단 완전한 LeavePool aggregate(기여 이벤트/만료 잡)는 여전히 `contributed_days` 단일 컬럼으로 대체 중.
