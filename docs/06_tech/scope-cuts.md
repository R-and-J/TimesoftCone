...
- 2026-05-28 — **FR-3.1 휴가 차감 우선순위** 구현(도메인 `planLeaveDeduction` 순수 + `UseLeaveUseCase`, ADMIN 엔드포인트). 외부 그룹웨어 트리거가 없어 미뤘던 항목 — 권위 있는 도메인 규칙을 미리 박아두는 게 더 안전하다고 판단, ADMIN 디버그/시연용으로 노출. 외부 통합 시 같은 use case 재사용.
