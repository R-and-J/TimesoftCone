...
> 사내 연차(휴가) 자산을 콘으로 환산해 경매·재분배·연말 배당하는 인하우스 시스템의 디자인 핸드오프 패키지입니다.
...
타임소프트(주) 직원의 **남은 연차**를 사내 가상 콘으로 환산해, **공용 풀에서 1일 단위로 경매** 처리하는 사내 플랫폼입니다. 낙찰자는 연차를 즉시 부여받고, 경매로 회수된 콘은 연말에 **지분 비례 배당**으로 직원에게 돌려줍니다.
