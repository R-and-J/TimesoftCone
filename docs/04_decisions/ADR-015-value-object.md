...
// 3. 도메인 표현력 부족 — 1콘과 1일은 같은 number지만 의미는 다름
```
...
| `Cone` | 콘 금액 | `>= 0` 정수 | 클래스 + 산술 메서드 (`add`, `subtract`) |
...
export class Cone {
  ...
  static of(value: bigint | number): Cone {
    const v = BigInt(value);
    if (v < 0n) throw new InvalidConeError('Cone cannot be negative');
    return new Cone(v);
  }

  static ZERO = Cone.of(0n);

  add(other: Cone): Cone { return Cone.of(this.amount + other.amount); }
  subtract(other: Cone): Cone {
    if (this.amount < other.amount) throw new InsufficientConeError();
    return Cone.of(this.amount - other.amount);
  }

  greaterThan(other: Cone): boolean { return this.amount > other.amount; }
  lessThanOrEqual(other: Cone): boolean { return this.amount <= other.amount; }
...
→ 도메인 코드가 `point.subtract(bid)` 로 자기-문서화. 음수 방지·차감 부족 검증이 *Cone 클래스에 응집*.
...
- **불변식 단일 위치 집중** — `Cone` 클래스 하나 보면 모든 규칙 확인
- **도메인 자기-문서화** — `point.subtract(bid)` vs `n - m`
- **테스트 표현력 향상** — `Cone.of(1000)` 같은 명시적 의도

### ⚠️ 트레이드오프

- **보일러플레이트** — 클래스 1개 ≈ 30~50줄 (`Cone`, `UserId` 등)
...
- **`new Cone(...)` 직접 호출 금지** — `Cone.of(...)` 정적 팩토리만 사용 (검증 강제)
...
1. **Phase 1** (반드시): `UserId`, `Cone`, `LeaveDays`, `AuctionId`, `LeaveType`
