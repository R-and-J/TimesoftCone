...
1. **회원 신원/조직 = ezpass 기준 (읽기)** — 이름·부서·직급(clsf)·직책(ofcsprtps)은 ezpass에서 가져온다. `role(EMPLOYEE/ADMIN)`은 ezpass **관리자 권한**(`mngrAuthorAt` = `mngr_author_no` 보유 여부)으로 매 로그인 재동기화. (직급/직책은 ezpass 토큰에 없어 `msaportal` 조회 필요; 부서명·관리자여부는 토큰에 있음.)
