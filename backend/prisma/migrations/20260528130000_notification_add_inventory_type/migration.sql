-- notification.type CHECK 제약에 INVENTORY_CREATED 추가(ADR-013 + ADR-017).
-- SQLite는 ALTER TABLE로 CHECK 수정 불가 → 테이블 재생성 + 데이터 보존.
CREATE TABLE "notification_new" (
    ...
    CONSTRAINT "notification_type_valid" CHECK ("type" IN ('OUTBID', 'AUCTION_WON', 'INVENTORY_CREATED')),
