...
**사용자 = 행위 기반 역할** (근속연수 아님): **직원**(공통 base)에서 판매자 · 구매자 · 관리자 역할이 파생
...
<span class="small">핵심 도메인은 **연차 경매** — 입찰·낙찰이 시스템의 심장이고, 나머지는 그 결과를 *반영·정산*하는 지원 기능. (우선순위 MoSCoW: M=Must / S=Should / C=Could)</span>

| 영역 | 기능 | 우선순위 |
|---|---|:---:|
| **경매 — 핵심** | **FR-2.1 실시간 입찰** — 포인트 한도 내, 상위 입찰 시 WebSocket 알림 | **M** |
| **경매 — 핵심** | **FR-2.2 낙찰 · 수익적립금 적립** — 포인트 차감 → 중앙 대장 누적 | **M** |
...
![h:470](img/class-core.png)

<span class="small">**핵심 요약** — 소유(컴포지션)·적립·기록 관계 + «service»/«Insert-Only» 스테레오타입 + 3-flag 상속. *전체 13클래스·5열거형 도면은 `docs/uml`.*</span>
...
<span class="small">**핵심 흐름** — 입찰: write 락 → 단일 트랜잭션(차감·최고가·기록) → 실시간 알림. 동기/비동기/반환 메시지. *Outbox·재시도 등 전체 도면은 `docs/uml`.*</span>
...
<span class="small">`Auction` 생애주기 — CREATED→OPEN→CLOSED→(낙찰)AWARDED /(유찰)UNSOLD→EXPIRED, `«choice»` 분기. *복합상태·entry/do/exit 전체 도면은 `docs/uml`.*</span>
...
**배당 계산 (지분 비례 + 나머지는 지분 1위에 가산)** <span class="small">— 아래 `escrow_balance` = 수익적립금 잔액</span>
```
1. raw      = escrow_balance × stake_ratio(user)
2. floor    = floor(raw)                       (정수 포인트 내림)
