...
<span class="small">**논리적으론 4개 서비스(MSA 구조)**, 실제 **배포는 단일 앱**.</span>
...
설계에 그치지 않고 **경매 입찰 → 낙찰 → 연말 배당**까지 핵심 경로 전 구간 구현 완료.
...
| 연말 배치·배당 | 완료 | 풀 수집(LeavePool)·Stake·배당 지급 — `배당 합 = 수익적립금` 검증·멱등 + 자동 스케줄러 |
...
| 마일스톤 | 작업 · 완료 기준 | 담당 |
|---|---|---|
| **M-A** 경매 핵심 | 입찰·낙찰 E2E — 동시 입찰 정합 100% | 오지석 · 김기철(DB 락) |
| **M-B** 연차·차감 | `AUCTION→EVENT→REGULAR` 강제 차감 통과 | 오지석 |
| **M-C** 연말 배치·배당 | 풀 수집·Stake·배당 — **배당 합 = 수익적립금** 검증 | 오지석 · 김기철(정합성) |
| **M-D** 관리자·UI | 유찰 EVENT 지급 · 화면·디자인 | 오지석 |
| **M-E** 외부 연동·시연 | ezpass/HR 연동 점검 + E2E 데모 | 김기철 · 공동(시연) |
