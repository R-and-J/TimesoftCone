...
경매의 입찰 재화와 배당 지급 형태가 [[ADR-009]]에서 "사내 복지 포인트"로 결정되어 있다. 그러나 **정책(어떤 화폐를 쓸지)** 과 **구조(어떻게 결합할지)** 는 별개의 관심사다.

만약 미래에 다음과 같은 변화가 발생하면:

- 사내 복지 포인트 정책이 폐지·교체됨
- 신규 화폐(예: "LEAVE COIN")로 전환 결정
- 본 시스템의 SaaS화로 회사마다 다른 화폐를 주입해야 함
- 다중 화폐 동시 운용 (복지 포인트 + 마일리지 등)
...
1. **DB 컬럼명·테이블명·API 엔드포인트가 코어에 침투** → 화폐 교체 시 도메인 코드 수정 필요
...
- [[ADR-009]] 복지 포인트 재활용 — 본 ADR과 분리된 *정책* 결정
- [[ADR-011]] 복지 포인트 시스템 자체 보유 — `WelfarePointProvider`의 구현 근거
